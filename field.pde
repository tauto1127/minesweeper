class Field{
    ...
    private void openSquare(int x, int y,Square[][] field){
        field[x][y].isOpened = true;
        OpenSquareRobot robo = new OpenSquareRobot(x,y,field);
        int[][] openedSquare = robo.getAutoOpenedSquare();
        for(int i = 0;i < field_Height; i ++){
            for(int j = 0;j < field_Width; j++){
                if(openedSquare[i][j] == 1){
                    field[i][j].isOpened = true;
                    try {
                        field[i-1][j-1].isOpened = true;
                    }catch (ArrayIndexOutOfBoundsException e) {
                        
                    }try {
                        field[i][j-1].isOpened = true;
                    }catch (ArrayIndexOutOfBoundsException e) {
    ...
    //private void checkAndSetSquare(int x, int y)
}
